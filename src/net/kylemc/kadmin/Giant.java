...
public class Giant implements org.bukkit.event.Listener
{
  ...
  protected void initItems() {
    ...
    ItemStack darmor = new ItemStack(Material.DIAMOND_CHESTPLATE, 1);
    ...
    List<String> lore = java.util.Arrays.asList(new String[] { org.bukkit.ChatColor.DARK_PURPLE + "Boots infused with gripping iron spikes" });
    im.setLore(lore);
    ...
    ItemStack crystals = new ItemStack(Material.PRISMARINE_CRYSTALS, 6);
    ...
    ItemStack shards = new ItemStack(Material.PRISMARINE_SHARD, 10);
  ...
  private void spawnGiant(CreatureSpawnEvent event) {
    ...
    if ((event.getEntityType().equals(EntityType.ZOMBIE)) && (event.getSpawnReason().equals(CreatureSpawnEvent.SpawnReason.NATURAL)) && 
      ...
      if (next == 5) {
        ...
        CraftGiant g = (CraftGiant)event.getEntity().getWorld().spawnEntity(l, EntityType.GIANT);
  ...
  private void giantDeath(EntityDeathEvent event)
  {
    if ((event.getEntityType().equals(EntityType.GIANT)) && 
      ...
      Bukkit.getServer().getWorld(event.getEntity().getWorld().getUID()).dropItem(event.getEntity().getLocation(), (ItemStack)dropTable.get(index));
  ...
  private void giantStomp(Location l, CraftGiant giant)
  {
  ...
  private void giantRoar(Location l) {
    List<Player> players = Bukkit.getServer().getWorld(l.getWorld().getUID()).getPlayers();
  ...
  private void giantToss(CraftGiant giant, Entity damager) {
    if ((damager instanceof Player)) {
      ...
      boolean dist = getDistance(player.getLocation(), giant.getLocation()) < 5.0D;
    ...
    throws InterruptedException
  {
    ...
    if (event.getEntityType().equals(EntityType.GIANT)) {
      ...
      if (event.getCause().equals(EntityDamageEvent.DamageCause.FALL)) {
        ...
        List<Player> players = Bukkit.getServer().getWorld(l.getWorld().getUID()).getPlayers();
        List<Player> close = new ArrayList<Player>();
  ...
  {
    double x2 = Math.pow(ent2.getX() - ent1.getX(), 2.0D);
    double y2 = Math.pow(ent2.getY() - ent1.getY(), 2.0D);
    double z2 = Math.pow(ent2.getZ() - ent1.getZ(), 2.0D);
  ...
  private void pushAway(Player player, double speed, Entity entity)
  {
    ...
    Vector unitVector = player.getLocation().toVector().subtract(entity.getLocation().toVector()).normalize();
  ...
  public void disableFirst(PluginDisableEvent event) {
    Plugin plug = Bukkit.getPluginManager().getPlugin("Multiverse-Core");
